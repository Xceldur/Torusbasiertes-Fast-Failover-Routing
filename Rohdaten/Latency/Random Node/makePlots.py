...
if __name__ == '__main__':
    ...
    for algo_name, result in results_evaluation:
        ...
        for e in result['absolute_failure_rates']:
            print(e)
        print('--- Latencys ---')
    ...
    sns.boxplot(x='it', y='failure_rate', hue='algo',
                data=data_pd, showfliers=False, medianprops={"linewidth": 2}, showmeans=True)
    plt.ylabel('unzustellbare Pings in %')
    plt.xlabel('Iteration (Ausgefallene Knoten)')
    plt.legend(title='Failover Algorithm')
    plt.savefig("plots/plots1.pdf", bbox_inches='tight', dpi=300)
    plt.show()
    ...
    sns.boxplot(x='it', y='latency_rate', hue='algo', data=data_pd,
                showfliers=False, whis=3, log_scale=2, medianprops={"color": "m", "linewidth": 2}, showmeans=True)
    plt.ylabel('Latenzraten in ms')
    plt.xlabel('Iteration (Ausgefallene Knoten)')
    plt.legend(title='Failover Algorithm')
    plt.savefig("plots/plots2.pdf", bbox_inches='tight', dpi=300)
    plt.show()
